...
Notes on attention masking:
  ...
  - SWA (sliding_window_size=W): apply an offset *banded* causal mask in all modes
    (prefill / decode-step / chunked) to enforce the window constraint.
...
class CausalSelfAttention(nn.Module):
    ...
    def _build_offset_swa_mask_bool(
        self, T_new: int, T_total: int, past_len: int, device
    ) -> torch.Tensor:
        """
        Boolean mask (T_new, T_total) where True means "allowed".

        Causal constraint (with cache offset):
          j <= past_len + i

        Sliding window constraint (window W):
          j >= (past_len + i) - (W - 1)
        ...
        window_allowed = j >= lb
        return causal_allowed & window_allowed

    def _build_offset_swa_mask_additive(
        self, T_new: int, T_total: int, past_len: int, device, dtype
    ) -> torch.Tensor:
        """
        Additive SWA mask (T_new, T_total) with 0 for allowed and -inf for disallowed.
        """
        allowed = self._build_offset_swa_mask_bool(T_new, T_total, past_len, device=device)
        mask = torch.zeros((T_new, T_total), device=device, dtype=dtype)
        mask = mask.masked_fill(~allowed, float("-inf"))
        return mask
    ...
    def forward(
        ...
        # - decode-step (T_new == 1 with past): no mask needed (unless SWA enabled)
        # - chunked decode (T_new > 1 with past): offset causal mask required
        need_mask = (past_kv is None) or (past_kv is not None and T_new > 1)
        use_swa = (self.sliding_window_size is not None)

        if self.flash:
            if use_swa:
                # SWA requires a banded causal mask in ALL modes (prefill/decode/chunked).
                attn_mask = self._build_offset_swa_mask_additive(
            ...
            if use_swa:
                allowed = self._build_offset_swa_mask_bool(T_new, T_total, past_len, device=x.device)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type: str, override_args: Optional[dict] = None) -> "GPT":
        ...
        for i in range(config.n_layer):
            ...
            # Robust transpose rule: HF (in, out) -> transpose to (out, in)
            w_full = w_hf.t()
            assert w_full.shape == (3 * config.n_embd, config.n_embd), (
                f"unexpected HF c_attn weight shape after transpose: {w_full.shape}"
            )
