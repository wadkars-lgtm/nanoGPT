...
def load_jsons(out_dir: str) -> List[Dict[str, Any]]:
    paths = sorted(glob(os.path.join(out_dir, "infer_*.json")))
    rows = []
    for p in paths:
        try:
...
def main():
    ...
    ap.add_argument("--prefix_cache_dir", default="bench_prefix")
    ...
    ap.add_argument("--debug_tail_lines", type=int, default=25, help="Tail N lines of child stdout on success when --debug.")
    ...
    batch_infer = os.path.join(root, "batch_infer.py")
    ...
    kv_modes = []
    for tok in args.kv_modes.split(","):
        t = tok.strip().lower()
        if not t:
            continue
        if t in ("true", "1", "t", "yes", "y"):
            kv_modes.append(True)
        elif t in ("false", "0", "f", "no", "n"):
            kv_modes.append(False)
        else:
